...
en iOS hace falta una Share Extension (ver `IOS-PENDIENTE.md`).
...
cobertura: **[docs/CATALOGS.md](CATALOGS.md)**.
- [`ARQUITECTURA.md`](ARQUITECTURA.md) — por dónde entrar al código: los paquetes, qué hace cada uno y la regla de dependencias.
- [`REFACTOR.md`](REFACTOR.md) — plan de refactor del código (no del dataset): qué se mide mal hoy, en qué orden se arregla y qué no hay que hacer.
- [`TOOLS.md`](TOOLS.md) — mapa de `tools/`: qué script usar para cada cosa, qué patrones conviven y qué quedó obsoleto.
...
cada frontera `expect/actual`, está en **[docs/IOS-PENDIENTE.md](IOS-PENDIENTE.md)** — escrito
